...
public class DatasetProcessorFactory {
	...
	static final Logger log = LoggerFactory.getLogger(DatasetProcessorFactory.class);
	
	private final ClassLoader clsLoader;
	
	public DatasetProcessorFactory(ClassLoader clsLoader) {
		super();
		this.clsLoader = clsLoader;
	}

	public Optional<DatasetProcessor> createInstance(
			String instClassifierFQN, Properties props) {
		Class<? extends DatasetProcessor> cls = loadClass(instClassifierFQN);
		DatasetProcessor result = createInstance(cls, props);
